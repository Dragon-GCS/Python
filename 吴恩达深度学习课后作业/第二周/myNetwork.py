...
def initialize_parameters(layer_dims):
    ...
    for l in range(1, L):
        parameter['W' + str(l)] = np.random.randn(layer_dims[l], layer_dims[l - 1]) * np.sqrt(2 / layer_dims[l - 1])
...
def model_backward(AL, Y, caches, hidden_activations):
    ...
    dAL = - (np.divide(Y, AL) - np.divide(1 - Y, 1 - AL)) / Y.shape[1]
    grads['dA2'], grads['dW3'], grads['db3'] = backward_propagation(dAL, caches[2],'sigma')
...
if __name__ == '__main__':
    ...
    parameters = multi_layers_model(X_train, Y_train, layers_dims, 'relu', learning_rate=0.1, iteration=20000)

    predict(X_train, Y_train, parameters, hidden_activations='relu')  # 训练集
    predict(X_test, Y_test, parameters, hidden_activations='relu')  # 测试集
